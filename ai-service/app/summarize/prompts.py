...
출력 언어는 한국어 고정(§5.3, 2026-07-05) — 영어 논문에서 영어 요약이 나오던 문제 수정.
전문용어·고유명사는 원어 유지를 허용한다.
"""
...
_PAPER_SYSTEM = (
    ...
    "코드펜스/설명/추가 텍스트 없이 JSON만. "
    "모든 값은 반드시 한국어로 작성한다 — 원문이 영어여도 한국어로 요약한다. "
    "단, 전문용어·고유명사·모델명은 원어를 그대로 써도 되고, "
    "glossary의 term은 원어를 유지한다.\n"
    f"스키마: {_PAPER_SCHEMA}\n"
...
_PLAIN_SYSTEM = (
    ...
    "모든 값은 반드시 한국어로 작성한다(전문용어·고유명사는 원어 유지 가능).\n"
    f"스키마: {_PLAIN_SCHEMA}\n"
