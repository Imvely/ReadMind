...
### 12.1 세션 시작 (항상 이 순서)
1. `claude-progress.md`를 읽어 직전 세션의 마지막 상태를 파악한다. (※ 진행 로그는 `.md`로 관리한다 — `.txt`는 사내 Fasoo DRM이 걸려 평문이 깨지므로 사용 금지.)
...
3. `claude-progress.md`에 한 줄 추가: `[날짜] <항목id> 완료 / 다음: <다음 항목id>`.
4. 모듈 완료 시 해당 `README`와 명세서 §8 체크박스 갱신.

### 12.5 강제 장치 (이미 걸린 hook — 우회 시도 금지)
> 원칙: **모델의 판단에 기대지 말고 결정론적 게이트로 강제한다.** 규칙은 가능하면 hook으로 기계화하고, hook이 못 잡는 것만 프로즈 규칙으로 남긴다. (어떤 모델로 작업하든 동일 품질이 나오게 하는 장치)

- **SessionStart(session-start.sh)**: 세션 시작 시 진행로그 최신 항목·미완료 feature·git 상태·검증도구 가용성을 컨텍스트에 자동 주입한다. "검증 게이트 비활성" 경고가 뜨면 코드 작업 전에 먼저 해소한다.
- **PreToolUse(guard.py)**: ① 시크릿 하드코딩(sk-/AIza/hf_/AKIA/private key 등 실키 포맷) ② `.env` 스테이징·커밋 ③ `.txt` 문서 생성(DRM — `.md`만, requirements*.txt 예외)을 차단한다. 차단되면 우회하지 말고 원인을 고친다.
- **PostToolUse(verify.py)**: 편집된 파일의 모듈만 골라 검증한다 — ai-service `*.py`→ruff, web/packages `*.ts(x)`→typecheck. **실패(exit 2) 피드백이 오면 다음 작업 전에 반드시 고친다.** backend `*.kt`는 편집 시점 검사가 없으므로 커밋 전 `./gradlew test`가 유일한 게이트다(§12.4).
- 이 hook들은 §3·§7·§10의 규칙을 기계적으로 강제하는 장치다. 통과를 우회·무력화하는 코드를 작성하지 않는다. 검증 도구가 없어 hook이 침묵하는 상태(집 PC 초기 등)를 "통과"로 착각하지 않는다.
